...
resp=$(curl -fsS -X PUT "https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/workers/scripts/${SCRIPT}" \
...
echo "$resp" | python3 -c "import json,sys; r=json.load(sys.stdin); assert r.get('success'), r; print('deploy ok:', r['result'].get('id'), 'size:', r['result'].get('bundleSize'))"
curl -fsS -X POST "https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/workers/scripts/${SCRIPT}/subdomain" \
